 ...
 */
  ...
  class MyWechat extends Wechat {
	...
	{ 
      $items = array(
        new NewsResponseItem("欢迎关注{$weixin_name}微信公众平台"),
        new NewsResponseItem("缺你运动微信平台将要获取您的：
    ...
    protected function onVoice() {
		$yuyin=$this->getRequest('Recognition');//识别之后的内容
		$yonghu=$this->getRequest('FromUserName');//发送人的openID
		$time=$this->getRequest('CreateTime');//获取消息创建的时间
	 
	$xuanze = "SELECT * FROM  `weixin_flag` WHERE  `openid` =  '{$yonghu}'";
	...
	$xuhao=mysql_fetch_row($query_num);*/
	$sql = "INSERT INTO `weixin_wall` (`id`,`messageid`,`fakeid`,`num`,`content`,`nickname`,`avatar`,`ret`,`datetime`) VALUES (NULL,'0','{$q['fakeid']}','-1','{$yuyin}','{$q['nickname']}','{$q['avatar']}','0','{$time}')";// ret 是审核，暂时设置为不需要审核
	mysql_query($sql);
	
	$q['nickname']=pack('H*',$q['nickname']);

    $this->responseText("{$q['nickname']}您发送的语音消息为：{$yuyin}语音消息需要审核通过才可以上墙。'");
